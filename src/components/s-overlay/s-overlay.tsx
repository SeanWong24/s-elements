...
@Component({
  tag: 's-overlay',
  styleUrl: 's-overlay.css',
  shadow: true,
})
export class SOverlay implements ComponentInterface {
  ...
  @Element() hostElement: HTMLSOverlayElement;
  ...
  showedChanged(showed: boolean, previousValue?: boolean) {
    if (showed) {
      ...
      const attachingContainer = document.querySelector('s-ui') || document.querySelector('body');
